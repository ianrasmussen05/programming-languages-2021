...
- The specification does not compile because it contains function declarations that are not implemented. Compilation (try it) will give you `lacks an accompanying binding` error messages.
...
- Start from the template file [`arithmetic.hs`](src/Assignment1/arithmetic.hs). You should be able to run it with `runhaskell arithmetic.hs`. Now add an implementation of `addP` plus the corresponding test cases in `main`.
...
**Task 2**: Extend the definition of `Exp` and `eval` in [`Interpreter.hs`](src/Calculator/Interpreter.hs) by 6 other operations on the integers including a binary `-`, a unary `-`, and `/` and `^` for integer division and exponentiation. Use the native Haskell operations on `Integer`.
